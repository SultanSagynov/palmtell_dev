...
const BASIC_ONE_TIME = process.env.LS_BASIC_ID;
...
if (BASIC_ONE_TIME) VARIANT_TO_PLAN[BASIC_ONE_TIME] = 'basic';
...
export async function createLemonSqueezyCheckout(data: CheckoutData) {
  ...
  if (plan === 'basic') {
    variantKey = 'basic_onetime';
  } else {
    variantKey = `${plan}_${interval === 'month' ? 'monthly' : 'annual'}`;
  }
